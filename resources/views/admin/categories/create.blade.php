...
@section('content')
    <div class="max-w-md">
        <div class="flex items-center gap-4 mb-6">
            ...
            <h1 class="text-xl font-semibold text-white">Tambah Kategori</h1>
        </div>
        <form action="{{ route('admin.categories') }}" method="POST" class="bg-gray-800/50 backdrop-blur-md border border-gray-700 rounded-2xl p-6 space-y-6">
            @csrf
            <div>
                <label class="block text-gray-300 text-sm mb-2">Nama Kategori *</label>
                <input type="text" name="name" value="{{ old('name') }}" class="w-full bg-gray-700 border border-gray-600 text-white rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500" required>
                @error('name')
